...
public class Args {
    ...
    public static void usage(Object target, String syntax) {
        usage(System.err, target, syntax);
    }
     ...
     * @param target An instance or class.
     */
    public static void usage(PrintStream errStream, Object target, String syntax) {
        Class<?> clazz;
        if (target instanceof Class) {
            clazz = (Class) target;
        } else {
        ...
        errStream.println("Usage: " + syntax);
        for (Class<?> currentClazz = clazz; currentClazz != null;  currentClazz = currentClazz.getSuperclass()) {
          for (Field field : currentClazz.getDeclaredFields()) {
